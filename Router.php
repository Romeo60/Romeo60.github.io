...
class Router {
    ...
    /**
     * If $default_route is passed, will call $this->init().
     * If $web_content_folder is passed, will call $this->loadAll().
     * 
     * @param string $default_route Name of default route, ie 'home'.
     * @param string $web_content_folder Relative path to folder containing routes.
     */
    function __construct($default_route = null, $web_content_folder = null) {
        $this->routes = array();
        $this->params = array();
        $this->default_route = $default_route === null ? "" : $default_route;
        $this->not_found_route = "not_found";
        $this->access_denied_route = "";
        $this->check_access_function = "";
        $this->web_content_folder = $web_content_folder === null ? "" : $web_content_folder;
        
        if($default_route !== null) {
            $this->init();
        }
        
        if($web_content_folder !== null) {
            $this->loadAll();
        }
    }
    ...
    function set($route_name, $controller, $args = null, $roles = null) {
    ...
    static function getIndex() {
        return (isset($_SERVER["HTTPS"]) ? 'https' : 'http')."://". $_SERVER['HTTP_HOST']. Router::getLocalIndex();
    }
    
    static function getUrl() {
        return (isset($_SERVER["HTTPS"]) ? 'https' : 'http')."://". $_SERVER['HTTP_HOST']. str_ireplace("index.php", "", $_SERVER['PHP_SELF']). ($_SERVER['QUERY_STRING']!='' ? "?". $_SERVER['QUERY_STRING'] : "");
    }
}